...
// cells are easy to locate. The drum-grid splits into a FIXED label column
// (its own canvas, outside the scroll viewport) + a CONTENT canvas inside a
// horizontal-scroll viewport (.drum-grid-vp), drawn from x=0 with NO gutter:
// RULER_H=20 px ruler, ROW_H=26 px per row. A click at (10, RULER_H+ROW_H/2) on
// the content canvas hits the first beat of the first voice row in draw mode
// (the default), which adds a note on an empty grid.
...
test('drum-grid pencil click is ONE undo step (no double-capture)', async ({ page }) => {
  ...
  // The drum editor's CONTENT canvas lives inside the scroll viewport
  // (#insp-roll-host .drum-grid-vp canvas). The first `#insp-roll-host canvas`
  // is now the fixed label column — we must target the content canvas.
  const canvas = page.locator('#insp-roll-host .drum-grid-vp canvas').first();
  ...
  // --- Step 3: Perform ONE pencil click on the content canvas to add a drum note ---
  // canvas.click() with a position offset: x=10 px (the content canvas has no
  // gutter — labels are a separate canvas), RULER_H + ROW_H/2 px down (centre of
  // the first voice row, below the ruler/scrub band). Triggers the canvas
  // pointerdown handler which calls pencilClick() in draw mode.
  await canvas.click({ position: { x: 10, y: RULER_H + ROW_H / 2 } });
