// plugins/delay/main.ts — ping-pong stereo delay. What it IS lives in
// plugin.json.
//
// Two delay lines of equal time cross-feed into each other — L's output feeds R
// and R's feeds L — so successive echoes alternate channels instead of piling
// up in the middle. The input enters the LEFT line, so the first repeat is
...
//
// One of the two inserts the host calls `setBpm` on (the tremolo is the other):
// a synced time is computed from the tempo rather than living on the knob.
import type { FxInstance } from '@loom/plugin-sdk';

// index → beats (0 = free / manual time)
const SYNC_BEATS = [0, 1, 0.5, 0.75, 1 / 3, 0.25, 1 / 6];

Loom.registerFx('delay', (ctx): FxInstance => {
