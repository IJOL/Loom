...
import { renderSessionGrid, renderSessionMixerStrip, type SessionUICallbacks } from './session-ui';
...
const slotEngineConfigurators: Array<(() => void) | null> = [null, null, null, null];
...
function rebuildMixer() {
  ...
  for (const t of activeTracks()) {
    const strip = stripFor(t);
    const state = strip.serialize();
    const col = document.createElement('div');
    col.className = `mix-col ${t}`;

    // ── Name header ──
    const name = document.createElement('div');
    name.className = 'mix-name';
    name.textContent = LANE_LABELS[t];
    col.appendChild(name);

    // ── EQ (HI / MID / LO stacked) ──
    const eqSec = document.createElement('div');
    eqSec.className = 'mix-section';
    const eqLab = document.createElement('div'); eqLab.className = 'mix-sec-label'; eqLab.textContent = 'EQ';
    eqSec.appendChild(eqLab);
    addMixKnobV(eqSec, { id: `mix.${t}.eqhi`,  label: 'HI',  min: -18, max: 18, step: 0.5, value: state.eqHigh, defaultValue: 0, color: '#2ee0c0', format: fmtDb, onChange: (v) => strip.setEqHigh(v) });
    addMixKnobV(eqSec, { id: `mix.${t}.eqmid`, label: 'MID', min: -18, max: 18, step: 0.5, value: state.eqMid,  defaultValue: 0, color: '#f7d000', format: fmtDb, onChange: (v) => strip.setEqMid(v) });
    addMixKnobV(eqSec, { id: `mix.${t}.eqlow`, label: 'LO',  min: -18, max: 18, step: 0.5, value: state.eqLow,  defaultValue: 0, color: '#c0392b', format: fmtDb, onChange: (v) => strip.setEqLow(v) });
    col.appendChild(eqSec);

    // ── Sends ──
    const sendSec = document.createElement('div');
    sendSec.className = 'mix-section';
    const sendLab = document.createElement('div'); sendLab.className = 'mix-sec-label'; sendLab.textContent = 'SEND';
    sendSec.appendChild(sendLab);
    addMixKnobV(sendSec, { id: `mix.${t}.rev`, label: 'REV', min: 0, max: 1, step: 0.01, value: state.reverbSend, defaultValue: 0, color: '#9b59b6', format: fmtPct, onChange: (v) => strip.setReverbSend(v) });
    addMixKnobV(sendSec, { id: `mix.${t}.dly`, label: 'DLY', min: 0, max: 1, step: 0.01, value: state.delaySend,  defaultValue: 0, color: '#3498db', format: fmtPct, onChange: (v) => strip.setDelaySend(v) });
    col.appendChild(sendSec);

    // ── Pan ──
    const panSec = document.createElement('div');
    panSec.className = 'mix-section';
    addMixKnobV(panSec, { id: `mix.${t}.pan`, label: 'PAN', min: -1, max: 1, step: 0.01, value: state.pan ?? 0, defaultValue: 0, color: '#e67e22', format: fmtPan, onChange: (v) => strip.setPan(v) });
    ...
    m.className = 'mix-btn mute'; m.textContent = 'M';
    if (muteState[t]) m.classList.add('active');
    m.addEventListener('click', () => {
      muteState[t] = !muteState[t];
      m.classList.toggle('active', muteState[t]);
      applyMuteSolo();
    });
    const s = document.createElement('button');
    s.className = 'mix-btn solo'; s.textContent = 'S';
    if (soloState[t]) s.classList.add('active');
    s.addEventListener('click', () => {
      soloState[t] = !soloState[t];
      s.classList.toggle('active', soloState[t]);
      applyMuteSolo();
    });
    ms.append(m, s);
    col.appendChild(ms);
...
function addMixKnobV(parent: HTMLElement, opts: {
  id?: string; label?: string;
  min: number; max: number; step: number; value: number; defaultValue?: number;
...
// ── Demo: Minimal Techno (4-pattern progression, all engines + automation) ─
...
function applyMinimalTechnoDemo() {
  ...
  slotEngineConfigurators[0] = null;                  // A: subtractive
  slotEngineConfigurators[1] = () => {                // B: wavetable on main
  ...
  slotEngineConfigurators[2] = () => {                // C: FM bells on main
    const inst = getLaneEngineInstance('main') as unknown as {
  ...
  slotEngineConfigurators[3] = null;                  // D: subtractive
  ...
  syncEngineToPattern();
  rebuildMixer();
  if (!chainEnabled) chainBtn.click();
  rebuildSynthTabs();
}

function wireDemoMinimalTechno() {
  const btn = document.getElementById('demo-minimal-techno') as HTMLButtonElement | null;
  btn?.addEventListener('click', applyMinimalTechnoDemo);
}
...
function renderSessionWithMixer() {
  ...
  renderSessionMixerStrip(sessionCallbacks._mixerRow, sessionState, {
    ...
    buildKnob: (paramId, label, container) => {
      ...
      // Clone the existing knob element so the Session strip shows current state
      // visually. The Classic Mixer panel remains the authoritative editor.
