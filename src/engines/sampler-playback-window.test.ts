...
describe('samplePlaybackWindow', () => {
  it('defaults play the whole buffer as a one-shot', () => {
    const w = samplePlaybackWindow(pad(), 2);
  ...
  it('trim shrinks the window and offsets the start', () => {
    const w = samplePlaybackWindow(pad({ sampleStart: 0.5, sampleEnd: 0.75 }), 2);
  ...
  it('loop uses [loopStart, loopEnd] in seconds and no fixed duration', () => {
    const w = samplePlaybackWindow(pad({ loop: 1, loopStart: 0.25, loopEnd: 0.75 }), 4);
  ...
  it('clamps a loop region to within the trim', () => {
    const w = samplePlaybackWindow(pad({ loop: 1, sampleStart: 0.2, sampleEnd: 0.6, loopStart: 0, loopEnd: 1 }), 10);
