...
// bridges those descriptors into the engine PLUGIN registry so the engine
// selector + preset id lists (which read listPlugins('engine')) keep working —
...
/** Bridge each registered engine descriptor into an engine PluginFactory so the
 *  engine selector + preset-id enumeration (listPlugins('engine')) see every
