...
export function wireDrumMasterUI(deps: DrumMasterUIDeps): void {
  ...
  mk({ id: `${laneId}.bus.sendA`, min: 0, max: 1, step: 0.01, value: state.sendA, defaultValue: 0,
    label: 'A', color: '#3498db', format: fmtPct, onChange: (v) => drumBusStrip.setSendA(v) });
  mk({ id: `${laneId}.bus.sendB`, min: 0, max: 1, step: 0.01, value: state.sendB, defaultValue: 0,
