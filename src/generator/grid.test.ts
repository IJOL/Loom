...
describe('the grid', () => {
  it('takes as many bars as repeats × 2^pow2', () => {
    expect(patternBars({ repeats: 1, div: 4, pow2: 0 })).toBe(1);
    expect(patternBars({ repeats: 3, div: 4, pow2: 0 })).toBe(3);
    expect(patternBars({ repeats: 3, div: 4, pow2: 2 })).toBe(12);
  });

  it('counts steps at whatever division it is handed', () => {
    const g = { repeats: 2, div: 4, pow2: 1 };
  ...
  it('clamps a grid out of range instead of refusing it', () => {
    expect(clampGrid({ repeats: 999, div: 99, pow2: 99 })).toEqual({ repeats: 16, div: 16, pow2: 3 });
    expect(clampGrid({ repeats: 0, pow2: -5 })).toEqual({ repeats: 1, div: 4, pow2: 0 });
  ...
  it('folds a step to a position inside the pattern', () => {
    const g = { repeats: 1, div: 4, pow2: 0 };
  ...
  it('comes round again exactly one pattern later', () => {
    const g = { repeats: 3, div: 4, pow2: 1 };
    const len = patternSteps(g, 4);
    for (let s = 0; s < 20; s++) {
  ...
  it('folds a NEGATIVE step to a real position, not a negative index', () => {
    ...
    const g = { repeats: 2, div: 4, pow2: 0 };
  ...
  it('reads the same absolute step the same way whenever it is asked', () => {
    const g = { repeats: 5, div: 4, pow2: 1 };
