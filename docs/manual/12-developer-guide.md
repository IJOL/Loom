...
**The golden-WAV loop runs on the worklet path.** `test/preset-render.dsp.test.ts` renders a fixed phrase for the first three presets of each melodic engine through `renderKernelLane` — the same pure kernel the offline export uses — and writes `test/output/`; `npm run test:wav-diff` compares that against the 18 WAVs committed in `test/golden/` and prints peak / RMS / L2 deltas (it never fails CI — it is a human inspection tool), and `npm run test:wav-bless` overwrites the golden set on purpose, so commit the result. Its sibling `test/preset-stability.dsp.test.ts` is the divergence detector over **every** shipped preset: finite, bounded, and the voice ends. (The pre-worklet batteries and their 90 orphan WAVs are gone; `test/dsp-battery.ts` no longer exists.)

**Two listening tools live in `tools/` and are gated on an environment variable**, so the regular sweep collects them and runs nothing. `tools/render-preset-auditions.test.ts` (`AUDITION=1`, `AUDITION_ENGINES=fm,westcoast`) renders every factory preset of the named engines four ways — plain, accented, a four-note chord, a long low note — raw and through the live master tail, and writes `test/output/auditions/` with an `index.html` listening page whose ratings export as JSON. It answers "does this **factory preset** render clean". `tools/render-session-lanes.test.ts` (`REPRO=1`, `REPRO_SAVE=/path/to/session.json`, required) answers "does **my lane**": for each melodic lane in a saved session it renders that lane's exact saved params with its own busiest clip at several polyphony caps (`REPRO_VOICES`, default `1,8,39`), raw and mastered, into `test/output/session-lanes/`. That is the reproduction shape for "sounds fine at minimum polyphony and turns to noise when Voices rises" — if the offline kernel reproduces the noise it is DSP, if it does not it is worklet-only. The session tool multiplies the manifest `outputTrim` by the synth category gain, as the host does; the audition tool applies the manifest trim alone, as the golden loop does.
