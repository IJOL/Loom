...
Three buttons on the transport bar drive session management:

| Button | What it does |
| --- | --- |
| **New** | Discards the current session and starts a blank one. |
...
See [Transport](02-transport.md) for the full transport bar layout.
...
The **⤓ WAV ▾** button in the transport bar exports the currently-playing scene to a stereo 16-bit WAV file. Clicking the button opens a small dropdown with two options:
...
- **A scene must be playing.** If no scene is active when you click either export option, Loom shows a brief notice and does nothing. Launch a scene first (see [Sessions](03-sessions-lanes-clips-scenes.md)), then export.
- **File name.** The download is named `loom-scene-<timestamp>.wav`.

After the export finishes the transport stops and the play button resets.
