...
`public/presets/drum-kits.json`. **The list is purely additive — it keeps everything the user had and adds the sample kits.** Three `<optgroup>`s: **GM** (the 8 GM-named `KIT *` presets, same names + kitIds as `drums-machine.json`), **Synth** (the 5 raw kit names — TR-909/TR-808/TR-606/CR-78/LinnDrum, the `DrumMachine` KITS), and **Samples** (the bundled sample kits):

```json
{ "presets": [
  { "name": "KIT Standard",      "group": "GM",      "kind": "synth",  "kitId": "909" },
  { "name": "KIT Room",          "group": "GM",      "kind": "synth",  "kitId": "linn" },
  { "name": "KIT Power",         "group": "GM",      "kind": "synth",  "kitId": "909" },
  { "name": "KIT Electronic",    "group": "GM",      "kind": "synth",  "kitId": "606" },
  { "name": "KIT TR-808",        "group": "GM",      "kind": "synth",  "kitId": "808" },
  { "name": "KIT Jazz",          "group": "GM",      "kind": "synth",  "kitId": "78" },
  { "name": "KIT Brush",         "group": "GM",      "kind": "synth",  "kitId": "78" },
  { "name": "KIT Orchestra",     "group": "GM",      "kind": "synth",  "kitId": "linn" },
...
Schema per entry: `name`, `group` (display heading), `kind` (`'synth'` | `'sample'`), and `kitId` (synth) / `drumkitId` (sample). All names are unique (the loader dedups by name). The GM names match `drums-machine.json`, so MIDI-import / `gm-lookup` recall — `factory:KIT Power` etc. — keeps resolving through the same unified lookup.
