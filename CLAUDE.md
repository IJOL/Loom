...
- **[src/instrument-presets/](src/instrument-presets/)** — the **preset surface** of the instrument page: `polysynth-presets.ts` (the preset dropdown + Randomize), `poly-preset-apply.ts`, `poly-preset-store.ts`. `polysynth.ts` and its node-per-note `PolySynth` class are **gone**, deleted with the worklet cutover — no engine implements `getPolySynth()`/`setPolySynth()` and nothing constructs one. What survives the name is `PolySynthParams`, the shape USER presets are serialized into under the `tb303-poly-presets-v1` localStorage key: **that key and that shape must not be renamed** (no migrations here). Every melodic lane, subtractive included, synthesises through `WorkletLaneEngine`.
- **"poly" vs "instrument"** — the page that paints the controls of every non-drum lane is `data-page="instrument"` (it was `poly`, after a class that no longer exists). `LaneEngineHostState.instrumentPageLaneId` is the lane painted on it — deliberately NOT the selected lane, since a drum lane routes to the drums page instead. Three things keep the old spelling on purpose: **`polyBlep`** (oscillator anti-aliasing, in the SDK DSP), **`polyphony`** (a plugin-manifest field — plugin ABI), and the **`poly` param-group id** in every `plugin.json`. Never sweep-rename "poly".
...
- **Add an engine — FIVE steps, not one.** Since the worklet cutover, "drop a file" is NOT enough: the glob makes the engine appear in the selector, but a lane using it stays SILENT at note time unless every step is done.
  1. `src/engines/<id>.ts` — declare the params as `EngineParamSpec[]` and register a **data-only descriptor**: `createDescriptorEngine({ id, name, polyphony, params, presets, modulators })` passed to `registerEngineFactory` + `registerEngine`. The descriptor carries metadata only; its `createVoice`/`buildParamUI` are inert. If `params` uses `group`, also declare that engine's group table as `EngineParamGroup[]` ([engine-param-groups.ts](src/engines/engine-param-groups.ts)) and pass it as `groups` — it is what says which sections exist, their title, order, colour, and which share a row; omit it and the grid falls back to first-appearance order, one row per group, no colour.
