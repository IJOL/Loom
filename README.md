...
- **9 instrument engines** — TB-303, Subtractive, FM, Wavetable, Karplus-Strong, **West Coast**, a **Sampler**, a **Drum machine** (synth kits *and* sample kits), and **Layers**, which holds up to four melodic engines in one lane, split or stacked, balanced by a single gesture.
...
- **Per-lane modulation** — **LFO**, **ADSR**, **S&H** and **Per-Note** modulators routable to any parameter. The last of those is a function of *which note this is* rather than of the clock, which is what stops a repeated line sounding machine-stamped.
- **WEAVE** — a third view beside Session and Performance, where a scene stops being fixed: each track **crossfades between loops** and goes on crossfading on its own, blending in scale degrees so a fade cannot detune. Write a per-track **list** of what it may draw from and in what order, let a track **follow** another and play what its harmony implies, or switch on **GEN** and have it read its own material through a pattern you dial instead of playing it back. **PRINT** freezes whatever you are hearing into ordinary clips.
- **FX & mixer** — one unified effect picker with **15 inserts** (multifilter, distortion, reverb, delay, compressor, limiter, tremolo, gate, chorus, flanger, phaser, bitcrusher, auto-wah, ring modulator, stereo width) insertable on any rack: per lane (audio included), on the master, or on the two general-purpose **Send A / Send B** return buses (seeded A = delay, B = reverb). Inserts are built from native Web Audio nodes (the synthesis itself runs in the AudioWorklet). Plus a mixer with **sidechain compression**. Any insert's params are modulation and Performance-automation destinations.
...
- **Plugin architecture** — most of what makes sound is **no longer in this repository's source**. Six of the melodic engines, all fifteen inserts, two modulators and the whole WEAVE view are **external drop-in plugins**: a directory under [`plugins/`](plugins/) with a `plugin.json` manifest, compiled by its own CLI into `public/plugins/<id>/` and loaded by the browser **at runtime**. They compile against the published SDK, [`@loom/plugin-sdk`](packages/loom-plugin-sdk/), and reach the host through one small runtime ABI — never through an import of this repo. There is no list to edit and no core file to touch; a manifest that declares itself is enough, and a plugin that throws is skipped and rolled back rather than taking the app down with it. See [docs/plugin-development.md](docs/plugin-development.md).
