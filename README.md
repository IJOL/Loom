...
- **Audio channel** — drop a WAV via **+ Audio** (or onto an audio-lane cell) and it plays **tempo-locked without pitch change** (WSOLA pitch-preserving time-stretch; at the loop's native BPM it's essentially the source untouched). The waveform shows as a header above the clip editor. Hit **Slice → pads** to chop the loop at its transients — from embedded **Acid / `cue` / AIFF** markers when present, otherwise onset + beat detection — into a **bank of one-shots** on a new Sampler lane with an auto-built note-per-slice clip that retriggers on the grid REX-style (pitch-natural, free-following the tempo). The slices become individually editable and remixable, with the source waveform kept as a header above the notes.
- **Session workflow** — arrange **lanes** that play **clips** in **scenes**, with per-clip piano-roll and drum-rack editors.
- **Clip loop regions** — loop just a slice of a clip: drag an A–B brace over a clip's editor to repeat, say, bars 2–3 while it plays, independent of every other clip. The brace lives on note and drum editors (including sliced loops, where only the hits inside the region fire); a pure audio channel plays whole, so **Slice → pads** first when you want per-region control.
